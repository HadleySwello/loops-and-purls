...
module.exports = {
  host: 'https://dry-ocean-97313.herokuapp.com/'
...
var patternHandlebars = function patternHandlebars(patternName, paragraph, dataId) {
  var patternHandlebarsTemplate = __webpack_require__(20);
  var context = {
    'patternName': patternName,
    'paragraph': paragraph,
    'dataId': dataId
  };
  var patternTemplate = patternHandlebarsTemplate(context);
  $('#empty-pattern-list').append(patternTemplate);
};

var chartHandlebars = function chartHandlebars(dataObj, headerRow) {
  var chartHandlebarsTemplate = __webpack_require__(37);
  var context = {
    'cellData': dataObj,
    'headerRow': headerRow
  };
  var chartTemplate = chartHandlebarsTemplate(context);
  $('#chart').append(chartTemplate);
};

var chartLogic = function chartLogic() {
  // chart constraints
  var height = 10;
  var width = 10;

  // generate default cell object
  var emptyCells = Array(width).fill('-');
  // emptyCells.unshift(' ')
  var dataObj = {};
  var headerRow = Array(width + 1).fill().map(function (value, i) {
    return i;
  });
...
module.exports = {
  patternHandlebars: patternHandlebars,
  chartHandlebars: chartHandlebars,
...
var getChartData = function getChartData() {
  var patternJson = {};
  patternJson['pattern'] = {};
  var rowData = $('#chart').find('tr:not(.headerRow)');
...
var getRowData = function getRowData(row) {
  var inputArray = $(row).find('td:not(.rowLabel) input');
  delete inputArray.prevObject;
  // console.log($(row))
  var inputValue = inputArray.map(function (index, input) {
    return $(input).val();
  });
...
var compilePattern = function compilePattern() {
  $('#empty-pattern-write-up').empty();
  var patternArray = [];
  for (var i = 1; i < 11; i++) {
    for (var j = 0; j < 10; j++) {
      var value = $('#row' + i + 'cell' + j).val();
      patternArray.push(value);
    }
  }
  for (var h = 0; h < patternArray.length; h++) {
    if (patternArray[h] === '-') {
  ...
  for (var _i = 0; _i < patternArray.length; _i++) {
    ...
    var prevVal = patternArray[_i - 1];
    var currVal = patternArray[_i];
    ...
    if (prevVal === currVal && ['Knit 1 ', 'Purl 1 '].indexOf(prevVal) > -1) {
      ...
      var accVal = prevVal.split(' ')[0] + ' ' + acc;
...
var concat = __webpack_require__(10);
...
var config = {
  apiOrigins: {
    production: 'https://dry-ocean-97313.herokuapp.com/'
  }
};
...
var onGetChartList = function onGetChartList(event) {
  // const data = getFormFields(this)
  event.preventDefault();
  api.getChartList().then(ui.getChartListSuccess).catch(ui.getChartListfailure);
};

var onDeleteChart = function onDeleteChart(event) {
  var dataId = this.getAttribute('data-id');
  event.preventDefault();
  api.deleteChart(dataId).then(ui.deleteChartSuccess).catch(ui.deleteChartFailure);
};

var onCreateChart = function onCreateChart(event) {
  event.preventDefault();
  var data = concat.getChartData();
  var pattern = JSON.stringify(data.pattern.chart);
  var paragraph = $('#empty-pattern-write-up').html();
  console.log(paragraph);
  api.createChart(data, paragraph, pattern).then(ui.createChartSuccess).catch(ui.createChartFailure);
};

var onUpdateChart = function onUpdateChart(event) {
  event.preventDefault();
  var dataId = this.getAttribute('data-id');
  var title = getFormFields(this).title;
  console.log('update dataId = ' + dataId);
  api.updateChart(dataId, title).then(ui.updateChartSuccess).catch(ui.updateChartFailure);
};
...
var deleteChart = function deleteChart(dataId) {
  return $.ajax({
...
var createChart = function createChart(data, paragraph, pattern) {
  console.log(paragraph);
  return $.ajax({
    url: app.host + '/patterns/',
    method: 'POST',
...
var updateChart = function updateChart(dataId, title) {
...
var getChartListSuccess = function getChartListSuccess(data) {
  console.log(data);
  ...
  for (var i = 0; i < data.patterns.length; i++) {
    var patternName = data.patterns[i].title;
    var paragraph = data.patterns[i].paragraph;
    console.log(patternName);
    var dataId = data.patterns[i].id;
    template.patternHandlebars(patternName, paragraph, dataId);
  }
};

var getChartListFailure = function getChartListFailure(error) {
  console.log(error);
  console.log('Nice Try Dude: getChartListFailure');
};

var deleteChartSuccess = function deleteChartSuccess(data) {
  console.log(data);
  console.log('Succeded in deleteChartSuccess');
};

var deleteChartFailure = function deleteChartFailure(error) {
  console.log(error);
...
var createChartFailure = function createChartFailure(error) {
  console.log(error);
  console.log('Nice Try Dude: createChartFailure');
};

var updateChartSuccess = function updateChartSuccess(data) {
  console.log(data);
  console.log('Succeded in updateChartSuccess');
};

var updateChartFailure = function updateChartFailure(error) {
  console.log(error);
...
module.exports = {
  ...
  getChartListSuccess: getChartListSuccess,
  getChartListFailure: getChartListFailure,
  deleteChartSuccess: deleteChartSuccess,
  deleteChartFailure: deleteChartFailure,
  createChartSuccess: createChartSuccess,
  createChartFailure: createChartFailure,
  updateChartSuccess: updateChartSuccess,
...
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
  ...
  return "<div>\n  <p data-id=\""
    ...
    + "\">"
    + alias4(((helper = (helper = helpers.patternName || (depth0 != null ? depth0.patternName : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"patternName","hash":{},"data":data}) : helper)))
    + "</p>\n  <p>"
    ...
    + "</p>\n  <button type=\"button\" class=\"patternDeleteButton btn-default\" data-id=\""
    + alias4(((helper = (helper = helpers.dataId || (depth0 != null ? depth0.dataId : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"dataId","hash":{},"data":data}) : helper)))
    + "\" name=\"deleteButton\">Delete</button>\n  <form id=\"update-title\" data-id=\""
    + alias4(((helper = (helper = helpers.dataId || (depth0 != null ? depth0.dataId : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"dataId","hash":{},"data":data}) : helper)))
    + "\">\n    <input name=\"title\" type=\"text\" placeholder=\"Update Title\"/>\n    <input type=\"submit\" class=\"btn-default\" value=\"Submit\">\n  </form>\n  <br></br>\n</div>\n";
},"useData":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "        <tr id=\""
    ...
    + "\">\n          <td class=\"rowLabel\">"
    + alias2(container.lambda((depth0 != null ? depth0.index : depth0), depth0))
    + "</td>\n"
    + ((stack1 = helpers.each.call(alias1,(depth0 != null ? depth0.data : depth0),{"name":"each","hash":{},"fn":container.program(4, data, 0, blockParams, depths),"inverse":container.noop,"data":data})) != null ? stack1 : "")
    + "        </tr>\n";
},"4":function(container,depth0,helpers,partials,data,blockParams,depths) {
    var helper, alias1=container.lambda, alias2=container.escapeExpression;

  return "            <td><input class=\"cell\" id=\"row"
    + alias2(alias1((depths[1] != null ? depths[1].index : depths[1]), depth0))
    + "cell"
  ...
  return "<div>\n  <form class=\"chart-form\">\n    <input id=\"title-input\" name=\"title\" placeholder=\"Title\">\n    <button type=\"submit\" id=\"clickMeh\" class=\"btn-default\" value=\"Submit\">Submit</button>\n    <br></br>\n    <table id=\"pattern-table\">\n      <tr class=\"headerRow\">\n"
    + ((stack1 = helpers.each.call(alias1,(depth0 != null ? depth0.headerRow : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0, blockParams, depths),"inverse":container.noop,"data":data})) != null ? stack1 : "")
    + "      </tr>\n"
    + ((stack1 = helpers.each.call(alias1,(depth0 != null ? depth0.cellData : depth0),{"name":"each","hash":{},"fn":container.program(3, data, 0, blockParams, depths),"inverse":container.noop,"data":data})) != null ? stack1 : "")
    + "    </table>\n  </form>\n</div>\n";
